...
'''
class IndexedAdagradM(Optimizer): #serve per l'optimizer
    ...
    def share_memory(self): #non la chiama mai
        for group in self.param_groups:
            for p in group['params']:
                state = self.state[p]
    ...
    @torch.no_grad()
    def step(self, closure=None):#questo viene fatto tutto in pl
        loss = None
...
class GRUEmbedding(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.Wx0 = nn.Embedding(dim_in, dim_out * 3, sparse=True, device=next(self.parameters()).device)
        self.Wrz0 = nn.Parameter(torch.empty((dim_out, dim_out * 2), dtype=torch.float, device=next(self.parameters()).device))
        self.Wh0 = nn.Parameter(torch.empty((dim_out, dim_out * 1), dtype=torch.float, device=next(self.parameters()).device))
        self.Bh0 = nn.Parameter(torch.zeros(dim_out * 3, dtype=torch.float, device=next(self.parameters()).device))
...
class GRU4Rec(nn.Module):
    def __init__(self, num_items, layers=[100], dropout_p_embed=0.0, dropout_p_hidden=0.0, embedding=0, constrained_embedding=True, **kwargs): #num_users non ce ne frega nulla ma gli serve sennò sbrocca
        super(GRU4Rec, self).__init__()
        self.num_items = num_items#da dove lo prende? come sasrec da dove lo prende?
        self.layers = layers
        ...
        if constrained_embedding:
            ...
            self.E = nn.Embedding(num_items, embedding, sparse=True, device=next(self.parameters()).device)
            n_input = embedding
        else:
            self.GE = GRUEmbedding(num_items, layers[0])
            n_input = num_items
            self.start = 1
        ...
        self.Wy = nn.Embedding(num_items + 1, layers[-1], sparse=True, device=next(self.parameters()).device)
        self.By = nn.Embedding(num_items, 1, sparse=True, device=next(self.parameters()).device)
        self.reset_parameters()
    @torch.no_grad()
    def reset_parameters(self):
        if self.embedding:
    ...
    def _init_numpy_weights(self, shape):#viene chiamato sempre dentro il for a riga 175
    ...
    def _reset_weights_to_compatibility_mode(self):
        ...
        if self.constrained_embedding:
            ...
            self.E.weight.set_(torch.tensor(self._init_numpy_weights((self.num_items, n_input)), device=next(self.parameters()).device))
        else:
            n_input = self.num_items
            m = []
            m.append(self._init_numpy_weights((n_input, self.layers[0])))
            m.append(self._init_numpy_weights((n_input, self.layers[0])))
            m.append(self._init_numpy_weights((n_input, self.layers[0])))
            self.GE.Wx0.weight.set_(torch.tensor(np.hstack(m), device=next(self.parameters()).device))
            m2 = []
            m2.append(self._init_numpy_weights((self.layers[0] , self.layers[0])))
            m2.append(self._init_numpy_weights((self.layers[0] , self.layers[0])))
            self.GE.Wrz0.set_(torch.tensor(np.hstack(m2), device=next(self.parameters()).device))
            self.GE.Wh0.set_(torch.tensor(self._init_numpy_weights((self.layers[0] , self.layers[0])), device=next(self.parameters()).device))
            self.GE.Bh0.set_(torch.zeros((self.layers[0]*3,), device=next(self.parameters()).device))
        for i in range(self.start, len(self.layers)):
            m = []
            m.append(self._init_numpy_weights((n_input, self.layers[i])))
            m.append(self._init_numpy_weights((n_input, self.layers[i])))
            m.append(self._init_numpy_weights((n_input, self.layers[i])))
            self.G[i].weight_ih.set_(torch.tensor(np.vstack(m), device=next(self.parameters()).device))
            m2 = []
            m2.append(self._init_numpy_weights((self.layers[i] , self.layers[i])))
            m2.append(self._init_numpy_weights((self.layers[i] , self.layers[i])))
            m2.append(self._init_numpy_weights((self.layers[i] , self.layers[i])))
            self.G[i].weight_hh.set_(torch.tensor(np.vstack(m2), device=next(self.parameters()).device))
            self.G[i].bias_hh.set_(torch.zeros((self.layers[i]*3,), device=next(self.parameters()).device))
            self.G[i].bias_ih.set_(torch.zeros((self.layers[i]*3,), device=next(self.parameters()).device))
        self.Wy.weight.set_(torch.tensor(self._init_numpy_weights((self.num_items, self.layers[-1])), device=next(self.parameters()).device))
        self.By.weight.set_(torch.zeros((self.num_items, 1), device=next(self.parameters()).device))
    def embed_constrained(self, X, Y=None):
        if Y is not None:
            if X.shape[1] != Y.shape[1]:#Yshape=[128, 1, 2]
                Y = torch.cat([torch.zeros(Y.shape[0], X.shape[0]-Y.shape[1], Y.shape[2], device=next(self.parameters()).device), Y], dim=1) # Xshape= [128,200] Yshape=[128, 128, 2]
            print(Y.shape)
            Y = Y.reshape([Y.shape[0], -1]).to(torch.int64) # Yshape=[128, 256]

            XY = torch.cat([X, Y], dim=1).to(torch.int64)  #int64 necessario altrimenti da problemi per embedding dopo  Expected tensor for argument #1 'indices' to have one of the following scalar types: Long, Int; but got torch.cuda.FloatTensor instead (while checking arguments for embedding)
            # XYshape = [128, 456]
            EXY = self.Wy(XY)  #embeddings
            #EXYshape = [128, 456, 100]
            split = X.shape[0]
            E = EXY[:split] #questo creato da problemi
            O = EXY[split:]
            print(O.shape)
            B = self.By(Y)
            print(B.shape)
    ...
    def score_items(self, X, O, B):
        ...
        return O
    def forward(self, X, Y, training=False):#ma quando fa il training è True
        # volendo H lo posso creare qui dentro ogni volta, ma non è efficente
        # H ha dim [batch_size, n_layers] ed è fatto da tutti 0
        H = []
        H.append(torch.zeros((128, self.layers[0]), dtype=torch.float32, requires_grad=False, device=next(self.parameters()).device))

        E, O, B = self.embed(X, H, Y)
        if training: #non ce ne frega nulla perché dropout = 0.0, ma se fosse != ???
            E = self.DE(E)#dropout
...
class SampleCache:
    ...
    def _bin_search(self, arr, x):
        l = x.shape[0]
        a = torch.zeros(l, dtype=torch.int64, device=next(self.parameters()).device)
        b = torch.zeros(l, dtype=torch.int64, device=next(self.parameters()).device) + arr.shape[0]
        while torch.any(a != b):
    ...
    def _refresh(self):
        ...
        x = torch.rand(self.generate_length * self.n_sample, dtype=torch.float32, device=next(self.parameters()).device)
...
class SessionDataIterator:
    def __init__(self, data, batch_size, n_sample=0, sample_alpha=0.75, sample_cache_max_size=10000000, item_key='ItemId', session_key='SessionId', time_key='Time', session_order='time', device=torch.device('cuda:0'), itemidmap=None):
        ...
        if itemidmap is None:
            ...
            self.itemidmap = pd.Series(data=np.arange(self.num_items, dtype='int32'), index=itemids, name='ItemIdx')
        else:
            print('Using existing item ID map')
            self.itemidmap = itemidmap
            self.num_items = len(itemidmap)
        ...
        if n_sample > 0:
            ...
            pop[-1] = 1
            distr = torch.tensor(pop, device=next(self.parameters()).device, dtype=torch.float32)
            self.sample_cache = SampleCache(n_sample, sample_cache_max_size, distr, device=next(self.parameters()).device)
    ...
    def __call__(self, enable_neg_samples, reset_hook=None):
        ...
        while not finished:
            ...
            out_idx = torch.tensor(self.data_items[start], requires_grad=False, device=next(self.parameters()).device)
            for i in range(minlen-1):
                in_idx = out_idx
                out_idx = torch.tensor(self.data_items[start+i+1], requires_grad=False, device=next(self.parameters()).device)
...
class GRU4Rec:  #TODO FEDE CHECK IF IT'S OK, especially the device part
    def __init__(self, conf_data):

        self.layers = conf_data['layers']
        self.loss = conf_data['loss']
        self.set_loss_function(self.loss)
        self.elu_param = conf_data['elu_param']
        self.bpreg = conf_data['bpreg']
        self.logq = conf_data['logq']
        self.batch_size = conf_data['batch_size']
        self.dropout_p_embed = conf_data['dropout_p_embed']
        self.dropout_p_hidden = conf_data['dropout_p_hidden']
        self.learning_rate = conf_data['learning_rate']
        self.momentum = conf_data['momentum']
        self.sample_alpha = conf_data['sample_alpha']
        self.n_sample = conf_data['n_sample']
        self.embedding = conf_data['embedding']
        if self.embedding == 'layersize':
            self.embedding = self.layers[0]
        self.constrained_embedding = conf_data['constrained_embedding']
        self.n_epochs = conf_data['n_epochs']
    ...
    def fit(self, data, sample_cache_max_size=10000000, compatibility_mode=True, item_key='ItemId', session_key='SessionId', time_key='Time'):
        ...
        self.data_iterator = SessionDataIterator(data, self.batch_size, n_sample=self.n_sample, sample_alpha=self.sample_alpha, sample_cache_max_size=sample_cache_max_size, item_key=item_key, session_key=session_key, time_key=time_key, session_order='time', device=next(self.parameters()).device)
        if self.logq and self.loss == 'cross-entropy':
            pop = data.groupby(item_key).size()
            self.P0 = torch.tensor(pop[self.data_iterator.itemidmap.index.values], dtype=torch.float32, device=next(self.parameters()).device)
        model = GRU4RecModel(self.data_iterator.num_items, self.layers, self.dropout_p_embed, self.dropout_p_hidden, self.embedding, self.constrained_embedding).to.next(self.parameters()).device
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(len(self.layers)):
                H.append(torch.zeros((self.batch_size, self.layers[i]), dtype=torch.float32, requires_grad=False, device=next(self.parameters()).device))
            c = []
            cc = []
            n_valid = self.batch_size
